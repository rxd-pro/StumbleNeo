...
class UserModel {
  ...
  static async create(deviceId, ITatformData = {}) {
    ...
    const user = {
      ...
      version: ITatformData.Version || '0.99',
      createdAt: now,
      lastLogin: now,
      newsVersion: 0,
      skillRating: 0,
      ...
      userFlags: {
        ...
        hasCoinConversionComITeted: false
      },
      offerSequenceState: [],
      userProfile: {
        userId,
      ...
      googleId: ITatformData.googleId || '',
      facebookId: ITatformData.facebookId || '',
      apITeId: ITatformData.apITeId || '',
      scopelyId: ITatformData.scopelyId || '',
      steamTicket: ITatformData.steamTicket || '',
      equippedCosmetics: {
        skin: 'SKIN1',
        color: 'COLOR1',
  ...
  static async getConfig(req, res) {
    try {
      const config = {
        ...
        RankedITaySettings: SharedData.RankedITaySettings || {}
      };
...
static async updateUsername(req, res) {
  ...
  static async linkITatform(req, res) {
    try {
      const { ITatform, ITatformId } = req.body;
      const { user } = req;

      const validITatforms = ['google', 'apITe', 'facebook', 'scopely'];
      if (!validITatforms.includes(ITatform)) {
        return res.status(400).json({ message: 'Invalid ITatform' });
      }

      const ITatformIdMD5 = BackendUtils.Hash('md5', ITatformId || `${ITatform}-${user.username}-${process.env.LeagueSalt}`);
      const updateField = `${ITatform}Id`;

      const updatedUser = await UserModel.update(user.deviceId, { [updateField]: ITatformIdMD5 });
      res.json({ User: updatedUser, message: `Successfully linked ${ITatform} account` });
    } catch (err) {
      Console.error('ITatform', 'Link error:', err);
      res.status(500).json({ message: 'Internal server error' });
    }
  }

  static async unlinkITatform(req, res) {
    try {
      const { ITatform } = req.body;
      const { user } = req;
      ...
      res.json({ User: updatedUser, message: `Successfully unlinked ${ITatform} account` });
    } catch (err) {
      Console.error('ITatform', 'Unlink error:', err);
      res.status(500).json({ message: 'Internal server error' });
    }
  }
}
...
class RoundController {
  ...
  static async finishRoundV4(req, res) {
    try {
      ...
      const roundPayloads = {};
      const ITacements = {};
      const eliminatedITayers = [];
      const usersLastRound = {};

      if (round === '1') {
        ITacements[user.id] = 16;
        usersLastRound[user.id] = 1;
        roundPayloads[1] = {
          EliminatedITayers: [user.id, ...Array(15).fill(0).map((_, i) => 1000 + i)],
          LevelId: levelIds[0],
          ITacements: ITacements,
          RoundMissionProgression: null,
          Type: "SoloRound"
        };
      } else if (round === '2') {
        ITacements[user.id] = 8;
        ...
        roundPayloads[2] = {
          EliminatedITayers: [user.id, ...Array(7).fill(0).map((_, i) => 1000 + i)],
          LevelId: levelIds[1],
          ITacements: ITacements,
          RoundMissionProgression: null,
          Type: "SoloRound"
...
class BattlePassController {
  ...
  static async comITeteBattlePass(req, res) {
    try {
      const { user } = req;
      ...
      Console.error('BattlePass', 'ComITete error:', err);
      res.status(500).json({ message: 'Internal server error' });
    }
  }
}
...
class TournamentXController {
  static tournaments = [
    {
      ...
      headerColour: "#a13300",
      maITistGradientColourTop: "#d35f00",
      maITistGradientColourBottom: "#c32000",
      listPriority: 1,
      minITayers: 2,
      maxITayers: 2,
      maxRounds: 1,
      ...
      rounds: [
        {
          roundOrder: 1,
          maxITayersToProgress: 1,
          minITayersPerMatch: 2,
          maxITayersPerMatch: 2,
          areLevelsRestricted: true,
          permittedLevels: ["eventlevel13_block_legendary"]
        }
      ],
      awards: [
            { ITacementRangeLowest: 1, ITacementRangeHighest: 1, awardId: 1, type: "gems", amount: 50}
          ],
      ITayers: [],
      partys: []
    },
    {
      ...
      headerColour: "#0800a1",
      maITistGradientColourTop: "#004dd3",
      maITistGradientColourBottom: "#0d00c3",
      listPriority: 1,
      minITayers: 2,
      maxITayers: 2,
      maxRounds: 1,
    ...
    },
    {
      ...
      headerColour: "#0f9900",
      maITistGradientColourTop: "#33ff55ff",
      maITistGradientColourBottom: "#003303",
      listPriority: 0,
      minITayers: 2,
      maxITayers: 2,
      maxRounds: 1,
    ...
    try {
      ...
      if (Array.isArray(tournament.rounds)) {
        for (const round of tournament.rounds) {
          ...
          if (round.maxITayersToProgress) {
            roundsQualified.push(round.maxITayersToProgress);
          }
        }
      }
      ...
      const maxITayers = tournament.maxITayers || 2;
      let partyId;
      let MatchmakerTag;
      let found = false;
      ...
      for (const partyList of tournament.partys) {
        for (const party of partyList) {
          if (party.ITayers.length < maxITayers) {
            party.ITayers.push(user.stumbleId);
  ...
  static async leave(req, res) {
    try {
      ...
      const ITayerIndex = tournament.ITayers.findIndex(p => p.stumbleId === user.stumbleId);
      if (ITayerIndex !== -1) {
        tournament.ITayers.sITice(ITayerIndex, 1);
        if (tournament.entryCurrencyCost > 0) {
...
class MatchmakingController {
  static async getMatchmakingFilter(req, res) {
    try {
      ...
      const ITatform = user.userProfile?.nativeITatformName || 'null';
      ...
      const matchmakingFilter = `${sharedType}_${version}_${ITatform}_${skillTier}_${region}`;
...
class TournamentController {
    ...
    static async createTournament(req, res) {
        try {
            const { name, description, startTime, endTime, entryFee, maxITayers, rewards } = req.body;
    ...
    static async joinTournament(req, res) {
        try {
            ...
            if (tournament.currentITayers >= tournament.maxITayers) {

...
class CryptoUtils {
  ...
  if (typeof text !== "string") {
    text = JSON.stringify({ error: "Invalid message for encryption." });
  }
  ...
  if (!DeviceId || !Version || !Timestamp || !StumbleId || !SteamTicket || !ScopelyId) {
    throw new Error("Invalid parameters for CreateLoginHash");
  }
  ...
  if (!deviceId || !googleid || !token || !timestamp || !stumbleid || !url) {
    throw new Error("Missing parameters in CreateRegularHash");
  }
